    ...
    tps && tps.length > 0
      ...
      : entregas
          .filter(
            (e) =>
              e.tp &&
              new Date(e.tp.fecha_entrega) > new Date() &&
              e.estado === "pendiente",
          )
          .slice(0, 3);

  const completedTpGrades = completedEntregas
      ...
      <div className="space-y-6">
        ...
        <div className="grid gap-6 lg:grid-cols-2">
          <Card>
            <CardHeader>
              ...
              <CardDescription>
                Trabajos prácticos con fecha de entrega próxima
              </CardDescription>
            </CardHeader>
            <CardContent className="space-y-4">
              {upcomingTps.length > 0 ? (
                upcomingTps.map((item) => {
                  ...
                  const fecha_entrega =
                    item.fecha_entrega || item.tp?.fecha_entrega;
                  const comision = item.comision || item.tp?.comision;
                  const estado = item.estado;

                  const daysUntil = Math.ceil(
                    (new Date(fecha_entrega).getTime() - new Date().getTime()) /
                      (1000 * 60 * 60 * 24),
                  );
                    ...
                    >
                      <div className="space-y-1 flex-1">
                        <p className="font-medium text-sm">
                          {comision?.materia?.nombre || comision?.nombre}
                        </p>
                        ...
                        <p className="text-xs text-muted-foreground">
                          Entrega:{" "}
                          {format(new Date(fecha_entrega), "dd 'de' MMMM", {
                            locale: es,
                          })}
                        </p>
                        {estado && (
          ...
          <Card>
            <CardHeader>
              ...
              <CardDescription>
                Últimas actualizaciones y avisos
              </CardDescription>
            </CardHeader>
            <CardContent className="space-y-4">
              {recentNotifications.length > 0 ? (
                recentNotifications.map((notif) => (
                  ...
                  >
                    <div
                      className={`mt-0.5 ${notif.leida ? "text-muted-foreground" : "text-primary"}`}
                    >
                      {notif.leida ? (
        ...
        <Card>
          <CardHeader>
            ...
            <CardDescription>
              Tus materias del año lectivo {new Date().getFullYear()}
            </CardDescription>
          </CardHeader>
          <CardContent>
            <div className="grid gap-4 md:grid-cols-2">
              {cursadas.map((cursada) => {
                const cursadaEntregas = entregas.filter(
                  (e) => e.cursada_id === cursada.id,
                );
                const completedCount = cursadaEntregas.filter(
                  (e) => e.estado === "calificado",
                ).length;
                const totalCount = tps.filter(
                  (tp) => tp.comision_id === cursada.comision_id,
                ).length;
                const progress =
                ...
                return (
                  <Link
                    key={cursada.id}
                    href={`/dashboard/student/materias/${cursada.id}`}
                  >

...
import { tpAPI, entregaTPAPI, cursadaAPI } from "@/lib/api"
import { Calendar, FileText, Upload, CheckCircle2, Clock, AlertCircle } from "lucide-react"
...
export default function TrabajosPage() {
  ...
  useEffect(() => {
    if (user && (user.role === "student" || user.role === "alumno")) {
      ...
      Promise.all([
        ...
        .then(([tpsResponse, entregasResponse, cursadasResponse]) => {
          console.log('Student TPs data:', { tpsResponse, entregasResponse, cursadasResponse })
          ...
          const cursadasList = cursadasResponse.data || cursadasResponse.cursadas || cursadasResponse || []
  ...
  const renderPendingTPCard = (tp: any) => {
    const daysUntil = Math.ceil((new Date(tp.fecha_entrega).getTime() - new Date().getTime()) / (1000 * 60 * 60 * 24))
        ...
        <CardHeader>
          <div className="flex items-start justify-between">
            <div className="space-y-1 flex-1">
              <CardTitle className="text-lg">{tp.comision?.nombre || `Comisión ${tp.comision_id}`}</CardTitle>
              <CardDescription className="line-clamp-2">{tp.consigna}</CardDescription>
            </div>
            {isOverdue ? (
        ...
        <CardContent className="space-y-4">
          <div className="flex items-center gap-2">
            ...
            <div>
              ...
              <p className="font-medium">{format(new Date(tp.fecha_entrega), "dd/MM/yyyy")}</p>
            </div>
          </div>

          <Dialog open={uploadDialogOpen && selectedTP?.id === tp.id} onOpenChange={setUploadDialogOpen}>
            ...
            <DialogContent>
              ...
              <div className="space-y-4 py-4">
                ...
                <Button onClick={handleUpload} className="w-full" disabled={!archivoUrl.trim() || submitting}>
    ...
    return (
      <Card key={entrega.id} className="hover:shadow-md transition-shadow">
        <CardHeader>
          <div className="flex items-start justify-between">
            <div className="space-y-1 flex-1">
              <CardTitle className="text-lg">{entrega.tp?.comision?.nombre || `TP #${entrega.tp_id}`}</CardTitle>
              <CardDescription className="line-clamp-2">{entrega.tp?.consigna}</CardDescription>
            </div>
            {entrega.nota ? (
              <Badge className="bg-green-500 hover:bg-green-600">Calificado: {entrega.nota}</Badge>
            ) : (
        ...
        <CardContent className="space-y-4">
          <div className="grid grid-cols-2 gap-4 text-sm">
            <div className="flex items-center gap-2">
              ...
              <div>
                ...
                <p className="font-medium">{format(new Date(entrega.fecha_entrega), "dd/MM/yyyy")}</p>
              </div>
            </div>
            {entrega.nota && (
              <div className="flex items-center gap-2">
          ...
          {entrega.devolucion && (
            <div className="p-3 bg-muted rounded-lg">
              <p className="text-sm font-medium mb-1">Devolución del profesor:</p>
              <p className="text-sm text-muted-foreground">{entrega.devolucion}</p>
            </div>
          )}
    ...
    <DashboardLayout>
      <div className="space-y-6">
        <div>
          ...
          <p className="text-muted-foreground">Gestiona tus entregas y consulta calificaciones</p>
        </div>
        ...
        {loading ? (
          ...
          <Tabs defaultValue="pending" className="space-y-4">
            ...
            <TabsContent value="pending" className="space-y-4">
              {pendingTPs.length > 0 ? (
                <div className="grid gap-4 md:grid-cols-2">{pendingTPs.map(renderPendingTPCard)}</div>
              ) : (
                <Card>
                  <CardContent className="flex flex-col items-center justify-center py-12">
                    <CheckCircle2 className="h-12 w-12 text-muted-foreground mb-4" />
                    <p className="text-lg font-medium">No hay trabajos pendientes</p>
                    <p className="text-sm text-muted-foreground">Todos tus trabajos están al día</p>
                  </CardContent>
                </Card>
              )}
            </TabsContent>
            ...
            <TabsContent value="submitted" className="space-y-4">
              {submittedTPs.length > 0 ? (
                <div className="grid gap-4 md:grid-cols-2">{submittedTPs.map(renderSubmittedTPCard)}</div>
              ) : (
                <Card>
                  <CardContent className="flex flex-col items-center justify-center py-12">
                    <AlertCircle className="h-12 w-12 text-muted-foreground mb-4" />
                    <p className="text-lg font-medium">No hay trabajos entregados</p>
                    <p className="text-sm text-muted-foreground">Aún no has entregado ningún trabajo</p>
                  </CardContent>
                </Card>
              )}
            </TabsContent>

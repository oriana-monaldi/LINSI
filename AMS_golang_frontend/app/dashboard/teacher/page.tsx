...
export default function TeacherDashboardPage() {
  ...
  const pendingEntregas = entregas.filter((entrega) => {
    ...
    const isProfessorTp = Number.isFinite(entregaTpId)
      ? professorTpIds.has(entregaTpId)
      : false;
    return (
      isProfessorTp &&
      (entrega?.estado === "pendiente" || entrega?.nota == null)
    );
  });
  const pendingGrades = pendingEntregas.length;
  const upcomingEvaluaciones = evaluaciones.filter(
        ...
        <div className="grid gap-6 lg:grid-cols-2">
          <Card>
            <CardHeader>
              ...
              <CardDescription>
                Trabajos que requieren tu revisión
              </CardDescription>
            </CardHeader>
              ...
              {loading ? (
                ...
                recentSubmissions.map((entrega) => {
                  ...
                  const tpTitle =
                    ...
                    tp?.nombre ||
                    (tp?.consigna
                      ? String(tp.consigna).split("\n")[0].slice(0, 80)
                      : `Trabajo ${entrega.tp_id}`);
                  return (
                    <div
                      key={entrega.id}
                    ...
                    >
                      <div className="space-y-1 flex-1">
                        ...
                        <p className="text-sm text-muted-foreground">
                          {tp?.comision?.nombre ||
                            `Comisión ${tp?.comision_id || entrega?.tp?.comision_id || "-"}`}
                        </p>
                      ...
                      <Link
                        href={`/dashboard/teacher/trabajos/${entrega.tp_id}`}
                      >
            ...
            {loading ? (
              ...
              <div className="grid gap-4 md:grid-cols-2">
                {comisiones.map((comision) => {
                  const comisionTPs = tps.filter(
                    (tp) => tp.comision_id === comision.id,
                  );
                  const comisionEvaluaciones = evaluaciones.filter(
                    (ev) => ev.comision_id === comision.id,
                  );
                  const pendingCount = pendingEntregas.filter((entrega) => {
                    const entregaComisionId = Number(
                      entrega?.tp?.comision_id ?? entrega?.tp?.comision?.id,
                    );
                    return (
                      Number.isFinite(entregaComisionId) &&
                      entregaComisionId === Number(comision.id)
                    );
                  }).length;
                  ...
                  return (
                    <Link
                      key={comision.id}
                      href={`/dashboard/teacher/materias/${comision.id}`}
                    >
                        ...
                        <CardContent className="space-y-3">
                          <div className="flex items-center justify-between text-sm">
                            <div className="flex items-center gap-2">
                              <Calendar className="h-4 w-4 text-muted-foreground" />
                              <span>
                                {comisionEvaluaciones.length} evaluaciones
                              </span>
                            </div>
                            {pendingCount > 0 && (
